...
| `drager_vista300` | Dräger Vista 300 + 麻酔器（2画面構成） | ✓ 登録済み |

### 両モニターの項目対応表

| レポート項目 | Bio-Scope AM140 | Dräger Vista 300 + 麻酔器 | 備考 |
|---|---|---|---|
| 心拍数 | `HR`（緑・大） | `HR`（左画面・緑） | `PR`（脈拍数）は使用しない |
| 収縮期血圧 | `BP の SYS` または P1/P2 収縮期 | `ART` または `NIBP` の上段 | 観血圧優先 |
| 平均血圧 | `BP の MAP` または P1/P2 平均 | `ART(MAP)` 括弧内の値 ※`125/101 (109)` 形式 | 括弧内が MAP |
| 拡張期血圧 | `BP の DIA` または P1/P2 拡張期 | `ART` または `NIBP` の下段 | |
| SpO2 | `SpO₂`（黄・大） | `SpO₂`（左画面） | |
| EtCO2 | `CO₂` の **ET** 値（In と分離表示） | 右画面 `CO₂` の大きい値 | AM140 は In/ET 分離のため ET を使用 |
| 1回換気量 | `VT` [mL] | 右画面 `VT`（L表示時は×1000） | |
| 分時換気量 | `MV` [L/min] | 右画面 `MV`（例: 4.4） | |
| 最高気道内圧 | 表示なし→ null | 右画面 `Paw`/`PIP`/`Peak` | |
| 呼吸数 | `RR` | 右画面の呼吸回数（例: 11） | |
| ISO ダイヤル | null（物理ダイヤル） | null（物理ダイヤル） | 画面表示なし |
| ISO In | `ISO In` [%] | 右画面 `Iso` 吸気側 [%] | |
| ISO Et | `ISO Ex` [%]（Ex=呼気） | 右画面 `Iso` 呼気側 [%] | AM140 は "Ex" 表記 |
| ガス流量 O2/Air | null（画面外） | null（画面外） | 流量計を目視 |
| FiO2 | `O₂` | 右画面 `O₂` 吸気側 | |
| 体温 | `TEMP` または `T1` [°C] | 左画面 `TEMP`/`T1`（橙） | 下部の参照範囲ラベルは無視 |
