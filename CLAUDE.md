...
画面は **シングルページ（SPA）**。ヘッダーにタブバーを組み込み、「撮影」「撮影記録」「トレンド」「患者管理」の4タブで切り替える。ヘッダーにはタブバーの上に **患者選択行** を設け、現在の対象患者を常時表示・切り替えできる。
...
│ ╚══════════════════════╝      │
│ 記録一覧                      │
│ 04/30 14:32  心拍数: 78 bpm  │  ← 複数項目選択時は横並び表示
│ 04/30 12:15  心拍数: 75 bpm  │
└──────────────────────────────┘
```
...
| `db_service.py` | `update_record(id, vital_data)` | バイタル値・患者情報の手動修正（UPDATE） |
...
| `patients.js` | `loadPatients()` / `renderPatientList()` | 患者一覧の読み込み・描画 |
...
| メソッド | パス | ボディ | 説明 |
|---|---|---|---|
| `PATCH` | `/api/records/{id}` | `VitalData` JSON | 全17項目 + 患者情報 + notes を上書き |
