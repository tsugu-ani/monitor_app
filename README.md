...
- **トレンド表示**: 任意の項目を選択し、指定期間の時系列グラフと一覧を確認できる
...
1 項目を選んで時系列の折れ線グラフと一覧を確認できます。

```
┌──────────────────────────────┐
│ 表示項目 [心拍数（bpm）▼]    │  ← 17 項目から選択
...
2. 「表示項目」で確認したいバイタル項目を選ぶ（変更と同時に自動更新）
3. 「開始」「終了」を変更して「更新」をタップすると期間を変えられる
4. グラフの点にタッチするとその時刻の値がツールチップで表示される
